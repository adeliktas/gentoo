...
inherit cmake git-r3 systemd tmpfiles toolchain-funcs greadme optfeature

MY_DN="openvas"

DESCRIPTION="Open Vulnerability Assessment Scanner (live git version for Greenbone GVM/GSM)"
...
LICENSE="GPL-2 GPL-2+"
SLOT="0"
KEYWORDS=""  # live ebuilds are unkeyworded by design
...
src_prepare() {
	...
	# QA-Fix: Remove -Werror compiler flag (common upstream issue)
	sed -i -e "s/-Werror//" "${S}"/CMakeLists.txt || die

	# QA-Fix: Remove !CLANG doxygen warnings (only needed for non-clang)
	if use doc; then
		if ! tc-is-clang; then
			local f
			for f in doc/*.in; do
